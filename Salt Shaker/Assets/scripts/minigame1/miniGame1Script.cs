...
public class miniGame1Script : MonoBehaviour {
	...
	void Update () {
		...
		if (madeCat) {
			cube1.GetComponent<miniGame1CubeTouch> ().xDirection = -0.1f;
			cube1.GetComponent<miniGame1CubeTouch> ().yDirection = 0.1f;
			cube2.GetComponent<miniGame1CubeTouch> ().xDirection = 0.1f;
			cube2.GetComponent<miniGame1CubeTouch> ().yDirection = 0.1f;
		} else {
